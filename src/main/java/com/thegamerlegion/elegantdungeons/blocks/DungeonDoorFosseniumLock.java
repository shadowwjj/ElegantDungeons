...
public class DungeonDoorFosseniumLock extends Block {
    public DungeonDoorFosseniumLock() {
    ...
    public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) {
        if(player.getHeldItemMainhand().getItem() == RegistryHandler.KEY_T2_BOSS.get() || player.getHeldItemOffhand().getItem() == RegistryHandler.KEY_T2_BOSS.get()) {
            ...
            if(player.getHeldItemMainhand().getItem() == RegistryHandler.KEY_T2_BOSS.get() && !player.abilities.isCreativeMode){
