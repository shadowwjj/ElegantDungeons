...
public class DungeonDoorLacumiteLock extends Block {
    public DungeonDoorLacumiteLock() {
    ...
    public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) {
        if(player.getHeldItemMainhand().getItem() == RegistryHandler.KEY_T1_BOSS.get() || player.getHeldItemOffhand().getItem() == RegistryHandler.KEY_T1_BOSS.get()) {
            ...
            if(player.getHeldItemMainhand().getItem() == RegistryHandler.KEY_T1_BOSS.get() && !player.abilities.isCreativeMode){
